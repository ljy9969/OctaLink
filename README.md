...
- [ ] `배포` **카카오 비즈 검수 재신청** *(난이도 2, 외부 1~3 영업일)* — 콘솔에서 `gender/age_range/birthday/birthyear/name/phone_number/account_email` 동의 항목이 "권한 없음" 상태. 비즈니스 정보 등록 + 검수 신청.
  ...
  - **🚨 0.6.0 실기기 카카오 로그인 무반응 (해결 필요)**: 0.6.0 AAB 비공개 트랙 업로드 완료. 에뮬레이터는 정상, 실제 폰 설치본은 카카오 로그인 버튼 클릭부터 반응 없음. 원인 추정 — **Google Play App Signing 으로 재서명된 릴리스 빌드의 키 해시가 비즈 앱(1453976) 카카오 콘솔에 미등록**. 조치: Play Console → 설정 → 앱 무결성 → "앱 서명 키 인증서" SHA-1 복사 → Base64 키 해시 변환 → 카카오 디벨로퍼스 → 비즈 앱 → 앱 설정 → 플랫폼 → Android → 키 해시 추가 (재배포 불필요, 즉시 반영). Firestore 의 `kakao:4891520650(이지연)` doc 유무와는 무관 — 카카오 토큰 발급 단계 전이라 회원 조회까지 도달조차 안 함
- [ ] `배포` **Play 콘솔 비공개 테스트 새 릴리스 게시** *(난이도 2, ~30분)* — 카카오 비즈 키 해시 등록 + 검수 통과 확인 후 진행. `versionCode` + `versionName` bump → `gradlew :app:bundleRelease` → 비공개 트랙 업로드 + 출시 노트 작성. (현재 0.6.0 까지 업로드됨, 키 해시 이슈 해결 후 다음 릴리스부터 정상 동작)
