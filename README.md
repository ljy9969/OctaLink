...
**Phase 2 카카오 OAuth + Firestore 활성화 완료.** Firebase Blaze 전환 + Cloud Functions(`kakaoSignIn` / `completeSignup` / `leaveMembership` / `rejoinMembership`) 배포 + Firestore Rules 배포 + RepositoryProvider Phase 2 토글 모두 끝. **카카오 로그인 → 가입 폼(Kakao nickname/phone 자동 prefill) → Cloud Function 으로 `members/{uid}` 생성 → role 자동 부여 → 메인 앱 진입** end-to-end 검증 완료. **Phase 2 후속 Repository(Attendance / SkillScore / Comment / Post / Tournament) 모두 Firestore 구현체 활성**. Post(좋아요·이미지 첨부 Phase A) + Comment(한 줄 코멘트 송수신) + Attendance(체크인 영속화 + 운영진 출결 검토 + 월간 캘린더 좌우 페이징) + SkillScore **풀스택 워크플로우 완성**(관장 직접 평가 `directApprove` + 코치 제안→관장 검토 `propose` + canonical = 시간 최신 / ProfileScreen 차트가 `skillScores` 직접 구독 → 콘솔 편집에도 즉시 반영). **Play Store 출시 직전 단계.** UI 완성 + 자체 OctaLink 브랜딩 완료 + Play 콘솔 기본 스토어 등록정보 완료(아이콘 / 피처 그래픽 / 폰·7"·10" 스크린샷 24장 / 앱 설명 / IARC 12+) + 타겟 API 35 + 비공개 테스트 트랙 첫 출시(0.2.0). 다음은 SkillScore/Post 룰·인덱스 재배포 + 카카오 비즈 검수 재신청 + TournamentRepository 영속화 + Play 콘솔 새 릴리스 게시.
...
- [ ] `배포` **Play 콘솔 비공개 테스트 새 릴리스 게시** *(난이도 2, ~30분)* — 카카오 검수 통과 확인 후 진행. `versionCode 3` + `versionName 0.3.0` bump → `gradlew :app:bundleRelease` → 비공개 트랙 업로드 + 출시 노트(스킬 평가 워크플로우, 출결 검토 캘린더 페이징, posts/comments) 작성

**[중요도 ★★]**
- [ ] `UX / 백엔드` **TournamentViewModel → TournamentRepository 영속화** *(난이도 3, 반나절)* — 추첨/결과가 여전히 in-memory. 챔피언 결정 시 `tournaments.create()` + 매치 결과를 `tournaments.setMatchWinner()` 로 동기화, 종료 시 `tournaments.finish()` 호출. `Match`(이름 기반 UI 모델) ↔ `MatchDoc`(memberId 기반) 매핑은 `membersByName` 역인덱스 사용. **토너먼트 히스토리의 선행 조건**
- [ ] `UX` **토너먼트 히스토리** *(난이도 2, ~3시간; 위 영속화 의존)* — 종료된 토너먼트 보관 + 회원별 전적 누적. `tournaments.observeAll()` 로 `finishedAt != null` 필터 → 시간순 리스트 + 회원 프로필에 전적 카드
- [ ] `배포` **푸시 알림 (FCM)** *(난이도 4, 1~2일)* — 수업 리마인더, 한 줄 코멘트 도착, 대진표 업데이트, 스킬 점수 확정 알림. Cloud Functions 트리거(`onDocumentCreated`) + 클라이언트 알림 핸들러(이미 `OctaLinkMessagingService` skeleton 있음) + 알림 권한 / 토큰 관리. 채널별 우선순위/그룹화
...
- [ ] `UX` **CommunityScreen Phase B — 영상 첨부** *(난이도 5, 2~3일)* — 30s 길이 cap + 720p re-encode (ffmpeg-kit) + ExoPlayer 재생 + Storage 50MB 룰. ffmpeg-kit AAR + Storage 비용 모니터링 별도 필요. Phase A(이미지) 검증 후 진행
