...
- [ ] `배포` **카카오 비즈 검수 재신청** *(난이도 2, 외부 1~3 영업일)* — 콘솔에서 `gender/age_range/birthday/birthyear/name/phone_number/account_email` 동의 항목이 "권한 없음" 상태. 비즈니스 정보 등록 + 검수 신청.
  ...
  - **🚨 실기기 카카오 로그인 무반응 (부분 진단 완료, 키 해시 등록 필요)**: 0.6.0 AAB 비공개 트랙 업로드 후 실제 폰 설치본은 카카오 로그인 버튼 클릭부터 반응 없음. 원인 — **Google Play App Signing 으로 재서명된 릴리스 빌드의 키 해시가 비즈 앱(1453976) 카카오 콘솔에 미등록**. 추가로 05-19 발견된 `Calling startActivity() from outside of an Activity context` 런타임 예외도 동시 원인일 수 있어 `AuthRepository.signInWithKakao(activity: Activity)` 시그니처로 0.6.1 에서 fix 완료. 다음 빌드 게시 전에 Play Console → 설정 → 앱 무결성 → "앱 서명 키 인증서" SHA-1 복사 → Base64 키 해시 변환 → 카카오 디벨로퍼스 → 비즈 앱 → 앱 설정 → 플랫폼 → Android → 키 해시 추가 (재배포 불필요, 즉시 반영). Firestore 의 `kakao:4891520650(이지연)` doc 유무와는 무관 — 카카오 토큰 발급 단계 전이라 회원 조회까지 도달조차 안 함
- [ ] `배포` **Play 콘솔 비공개 테스트 0.6.1 게시** *(난이도 2, ~30분)* — 카카오 비즈 키 해시 등록 + 0.6.1 출시 노트 작성. 현재 0.6.0 까지 업로드됨, 0.6.1 (versionCode 7) AAB 는 로컬 빌드 완료 — 키 해시 등록 후 업로드. 출시 노트에 추가 항목: 카카오 로그인 Activity context fix, 스킬 점수 정밀도 round, 프로필 설정 화면 분리, 가입 승인 큐 성별 노출
