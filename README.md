...
- [ ] `배포 준비` **Firebase 룰/Functions 재배포** — `firebase deploy --only firestore:rules,storage,functions`. storage 룰 신규 (Storage 콘솔에서 bucket 활성화 선행), firestore 룰 attendance update 분기 + posts 룰 신규, functions 에서 `seedTestData` 소스 제거됨. 운영자 후속: 이미 배포된 `seedTestData` 는 `firebase functions:delete seedTestData --region asia-northeast3` + 테스트 회원 6개 doc 콘솔 수동 정리
- [ ] `백엔드 / 데이터` **후속 Repository 3개 — CommentRepository / SkillScoreRepository / TournamentRepository** — AttendanceRepository / PostRepository(05-14 완료) 와 동일 패턴(인터페이스 + InMemory + Firestore + ViewModel + 화면 와이어링 + Rules/Indexes). 코멘트 / 스킬 점수(PROPOSED→APPROVED 워크플로) / 토너먼트 결과가 실제 Firestore 로 영속화
- [ ] `UX 디테일` **AdminScreen — 운영진 공통 카드 활성화** — 현재 5개 항목 모두 "준비 중" placeholder. (1) 회원 한 줄 코멘트 작성 ← CommentRepository 의존, (2) 토너먼트 추첨/대진 관리 ← TournamentRepository 의존, (3) 스킬 점수 입력 PROPOSED ← SkillScoreRepository 의존. *(출결 검토는 AttendanceScreen 운영진 모드, 공지 작성은 CommunityScreen 공지 작성 으로 이미 진입 가능 — 중복 카드 정리 검토)*. 각 항목 onClick + 다이얼로그/라우팅 + `role.isStaff` 가드
