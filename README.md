...
- [x] (05-31~06-01) `iOS` **iOS 클라이언트 스캐폴딩 (Phase 1, Xcode 비의존 순수 Swift) — `d:/source/JEON2/OctaLink-iOS/` 별도 위치** — Android `OctaLink/` 와 형제, 백엔드(Cloud Functions / Firestore / rules / 데이터 모델) 100% 공유, **클라이언트만** 신규.
  - **36 Swift 파일 산출**: Models 12 (Belt / WeightClass / SkillSet / Schema = Role·MemberDoc·FirestoreCollections / Attendance / Comment / SkillScore / Post + PostTag + PostVisibility / Tournament + Match + Round / Routine + Day + Drill / Mission / Notification / AvatarCatalog). Firestore 호환성 — 모든 enum rawValue 가 Android Kotlin `.name` (대문자) 과 일치해 양 플랫폼 도큐먼트 양방향 디코드.
  - **Auth 4 + Domain 4**: RoleAllowlist / AiRoutineAccess (`canUseAiRoutine(role:member:)`) / KakaoIdentity (카카오 비즈앱 7 동의 항목 풀 셋) / SessionState + `@MainActor SessionStore` ObservableObject (uid + member 조합 → 4단계 phase). DateHelpers (`isoWeekKey` / `monthsSinceJoin` / `todayKSTString` — Cloud Function 과 비트 정확한 정합) / Schedule (KST 타임존 고정 + checkInWindow `[-30분, +10분]` + classSlotKey + validClassReminderKeys = 20개) / Signup 검증 / MaskingHelpers (서버 `maskId`/`maskName` 동일 규칙).
  - **Theme 6 + Components 1**: Palette (12 태그 색) / BeltColors (다크/라이트 분기 — WHITE/BLACK/UNKNOWN 어댑티브, 중간 채도는 단일) / Colors (브랜드 + AppColor 시멘틱 + `Color(light:dark:)` 어댑티브 이니셜라이저) / Typography (Material 3 → SwiftUI Font 6 롤) / Spacing (8pt 그리드 7단계) / Shapes (CornerRadius 6단계 + 카드 모디파이어). AvatarView — `UIImage(named:)` nil 시 accent 색 원형 + initial 글자 폴백 (자산 미배치 단계도 화면 안 깨짐).
  - **Repository 프로토콜 10개 풀 셋**: Member / Auth / Attendance / Tournament / Post / PostComment / SkillScore / WeeklyMission / WeeklyRoutine / Comment. 비동기 모델 = `AsyncThrowingStream<T, Error>` (실시간) + `async throws -> T` (one-shot). 체크인은 Cloud Function 위임 명시.
  - **자산 이전 가이드** (`docs/ASSETS_MIGRATION.md`): Android `drawable-nodpi/` 의 12 PNG (아바타 10 + logo + mark) → iOS `Assets.xcassets/{name}.imageset/` 1x/2x/3x 분할 절차. **PIL/스크립트 자동 리사이즈 금지** 명시 (브랜드 자산 손제작 원칙).
  - **iOS WBS 환경 가이드 확장**: Mac/iOS 기기 미보유 운영자 위한 **클라우드 Mac (MacinCloud)** 경로 — Standard 월 $30 + Apple Dev 연 $99 + 시뮬레이터 + TestFlight (베타 테스터 5명의 iOS 실기기 위임) = **iPhone 0대로 베타 출시까지 가능**.
  - **현재 상태**: Xcode 비의존 순수 Swift 만 작성 (UIKit/SwiftUI/Combine/Foundation 외 의존 0). Mac/Xcode 확보 시 그대로 드래그 → 즉시 빌드. Phase 1 의 Repository 구현체(`FirestoreXxx` / `FirebaseAuthRepository`)는 Xcode + SPM 의존성 추가 후 작성 단계.
...
> 🖥️ **전제 — iOS 빌드는 macOS + Xcode 가 필수.** 지금 작업 머신은 Windows 라 Claude 는 iOS 앱을 **컴파일·실행·화면 검증할 수 없다.** "완료"에 도달하려면 셋 중 하나가 있어야 한다: (a) **Mac + Xcode** — 운영자가 빌드/시뮬레이터 실행, Claude 는 코드 ↔ 빌드오류 핑퐁, (b) **클라우드 Mac 임대 (MacinCloud Standard 월 $30 등)** — 원격 데스크탑으로 Xcode 사용, Mac/iPhone 0대로도 출시 가능 (운영자가 본 단계 선택, 05-31), 또는 (c) **macOS CI** (GitHub Actions macOS runner / Xcode Cloud) — Claude 가 push → CI 빌드 로그로 이터레이트. 이게 없으면 산출물은 **미검증 Swift 스캐폴딩**까지이고 빌드·UI 검증은 불가.

> 💰 **Mac/iOS 기기 0대 출시 예산** (05-31 가이드): MacinCloud Standard 3개월 $90~120 + Apple Developer Program 1년 $99 = **약 $200 (≈27만 원)**. 실기기 검증은 시뮬레이터 + TestFlight (비공개 테스터 5명 중 iOS 사용자 위임) 으로 우회. 정식 출시 직전 통합검증용 중고 iPhone SE 3 1대 (30만 원) 는 선택. 진행 절차 + 시뮬레이터 커버리지 + 시작 체크리스트는 별도 채팅 가이드 참조.
