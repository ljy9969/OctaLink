...
- [ ] `배포 준비` **Play 콘솔 비공개 테스트에 2.aab 로 새 릴리스 게시** — 현재 비공개 테스트 트랙은 0.2.0 라벨이지만 첨부된 번들이 versionCode 1 (0.1.0) 이라 테스터 폰에 옛 빌드가 깔림. 라이브러리에 떠 있는 `2.aab (0.2.0, versionCode 2)` 로 새 릴리스 생성 → 출시 노트 입력(`<ko-KR>` 태그 포함) → "테스터를 위한 출시 시작" → Google 검토 큐. 검토 통과 후 versionCode 2 가 자동으로 1 을 supersede
- [ ] `UX 디테일` **AttendanceScreen — 회원/일자별 출결 검토 (운영진 모드)** — 현재 `운영진 모드` 카드는 "준비 중 — Firestore + Auth 도입 후 활성화" placeholder. 운영진(코치+관장+창조자) 진입 시: (a) 회원 선택 (드롭다운/검색) → (b) 일자 범위 picker → (c) 해당 회원의 attendance 서브컬렉션 조회 (`members/{id}/attendance` orderBy classDate DESC) → (d) 일자별 출석 여부 + 체크인 시각 + verified 토글 (운영진이 직접 정정 가능). `members/{id}/attendance` read 권한은 isStaff 또는 본인만으로 룰 보강 필요
- [ ] `UX 디테일` **CommunityScreen — 글 쓰기 (사진/영상 포함) + 공지 작성** — (1) 전 회원: 일반 글 작성 다이얼로그 (제목 / 본문 / 태그 칩 / 이미지·영상 첨부 0~N). (2) 운영진(`isStaff`): 공지 카테고리 추가 가능 (다른 글보다 상단 고정). (3) 미디어 업로드는 Firebase Storage 또는 Cloudinary — **사이즈 제한 필수** (이미지 5MB / 영상 30s @ 50MB 권장). 클라이언트 사전 압축(이미지 1920px max, 영상 720p re-encode) + 서버 사이즈 검증(Cloud Function trigger 거부). Blaze 요금 영향 큰 영역이므로 출시 직후 모니터링 셋업 동반
- [ ] `UX 디테일` **AdminScreen — 운영진 공통 카드 실제 동작** — 현재 카드 5개 항목 모두 "준비 중" placeholder. 활성화 순서: (1) 회원 한 줄 코멘트 작성 ← CommentRepository 의존, (2) 출결 검토 ← 위 AttendanceScreen 출결 검토와 통합, (3) 토너먼트 추첨/대진 관리 ← TournamentRepository 의존, (4) 공지 작성 ← 위 CommunityScreen 공지 와 통합, (5) 스킬 점수 입력 (PROPOSED) ← SkillScoreRepository 의존. 각 항목별 onClick 핸들러 + 해당 화면/다이얼로그 라우팅 + 권한 더블체크 (`role.isStaff` 사전 가드)
- [ ] `백엔드 / 데이터` **후속 Repository — CommentRepository / SkillScoreRepository / TournamentRepository / PostRepository** — AttendanceRepository 와 동일 패턴(인터페이스 + InMemory + Firestore + ViewModel + 화면 와이어링 + Rules/Indexes). 코멘트 / 스킬 점수(PROPOSED→APPROVED 워크플로) / 토너먼트 결과 / 커뮤니티 글이 실제 Firestore 로 영속화
- [ ] `UX 디테일` **토너먼트 히스토리** — 종료된 토너먼트 보관 + 회원별 전적 누적. TournamentRepository 완료 후 진행
- [ ] `배포 준비` **푸시 알림 (FCM)** — 수업 리마인더, 한 줄 코멘트 도착, 대진표 업데이트. Cloud Functions 트리거 + 클라이언트 핸들러 + 알림 권한/토큰 관리
